...
sessionRouter.post(
 ...
 // Update Session details for an existing session
 ...
 sessionRouter.put(
  "/update-session",
    ...
    if (Session) {
      ...
      res.send({ message: "Session details has been successfully updated!", Session: updatedSession });
    } else {
 ...
 sessionRouter.put(
  "/BecomeMentorForexisiting-session",
    ...
    if (Session) {
      ...
      const updatedSession = await Session.save();
      res.send({ message: "Now you are also a mentor for this session!", Session: updatedSession });
    } else {
      res.status(404).send({ message: "Session Not Found" });
    }
  })
...
sessionRouter.put(
  "/Registerfor-session",
    ...
    if (Session) {
      ...
      const updatedSession = await Session.save();
      res.send({ message: "You have been successfully registered for this session!", Session: updatedSession });
    } else {
      res.status(404).send({ message: "Session Not Found" });
    }
  })
  ...
  "/Unregister-mentor-session",
    ...
    if (Session) {
     ...
     const index = Session.Organizers.indexOf(req.body.userID);
      ...
      const updatedSession = await Session.save();
      res.send({ message: " Session appointment cancelled . You are no longer a Mentor for this session!", Session: updatedSession });
      }
      else
      {
        res.status(404).send({ message: "You are not booked for this session" });
      }
    } else {
      res.status(404).send({ message: "Session Not Found" });
    }
  ...
  "/Unregister-mentee-session",
    ...
    if (Session) {
     ...
     const index = Session.Participants.indexOf(req.body.userID);
      ...
      const updatedSession = await Session.save();
      res.send({ message: "You have been unregistered from this session!", Session: updatedSession });
      }
      else
      {
        res.status(404).send({ message: "You are not booked for this session" });
